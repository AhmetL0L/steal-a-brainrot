...
-- Eşyaları Al Butonu Ayarları
local grabItemsButtonWidth = 120
local grabItemsButtonHeight = 40
local grabItemsButtonPosX = 0.5
local grabItemsButtonOffsetFromCenter = -70 -- Serverhop butonunun soluna gelecek şekilde
...
grabItemsButton.BackgroundColor3 = Color3.fromRGB(0, 120, 0) -- Yeşil renk
...
grabItemsButton.Parent = serverHopButtonGui -- Aynı ScreenGui'ye ekleyebiliriz
...
grabItemsButton.MouseButton1Click:Connect(function()
    ...
    local success1, result1 = pcall(function()
        return CoinsShopServiceRequestBuy:InvokeServer("Quantum Cloner")
    end)

    if success1 then
        print("Quantum Cloner alımı başarılı: ", result1)
    else
        warn("Quantum Cloner alımı başarısız: ", result1)
    end

    local success2, result2 = pcall(function()
        return CoinsShopServiceRequestBuy:InvokeServer("Invisibility Cloak")
    end)

    if success2 then
        print("Invisibility Cloak alımı başarılı: ", result2)
    else
        warn("Invisibility Cloak alımı başarısız: ", result2)
    end
...
local function loopspeed(args, speaker)
    ...
    -- The second argument check was redundant, simplified to use the first argument as speed
